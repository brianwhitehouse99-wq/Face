...
export default function FinalScreen({ correct, total, totalScore, athleteIds, sportFilter, athletes, results, onReplay }: Props) {
  ...
  const messages = [
    "Tough one — check back tomorrow!",
    "Not bad, keep at it!",
    "Nice game!",
    "Perfect score! You really know your athletes.",
  ]
    ...
    <div className="min-h-screen bg-zinc-950 text-white px-4 py-10">
      <div className="w-full max-w-lg mx-auto text-center">
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl px-8 py-6 mb-6">
          <div className="text-6xl font-black tracking-tighter text-yellow-400 mb-1">
          ...
          <div className="border-t border-zinc-800 mt-4 pt-4 flex justify-center gap-8">
            ...
            <div>
              ...
              <div className="text-zinc-500 text-xs">of max score</div>
            </div>
          </div>
        </div>
              ...
              {athletes.map((athlete, i) => (
                <div key={athlete.id} className={`rounded-xl overflow-hidden border ${results?.[i] ? 'border-yellow-400' : 'border-zinc-700'}`}>
                  <div className="relative w-full h-28">
                    ...
                    />
                    {results?.[i] && (
                      <div className="absolute top-1.5 right-1.5 bg-yellow-400 text-black text-xs font-bold w-5 h-5 rounded-full flex items-center justify-center">✓</div>
                    )}
                    {!results?.[i] && (
                      <div className="absolute top-1.5 right-1.5 bg-red-500 text-white text-xs font-bold w-5 h-5 rounded-full flex items-center justify-center">✗</div>
                    )}
                  </div>
        ...
        {!challengeLink && !showChallenge && (
          <div className="flex flex-col gap-3">
            <button
              ...
              className="w-full py-4 bg-yellow-400 hover:bg-yellow-300 active:scale-[0.98] text-black font-bold text-base rounded-xl transition-all"
            >
              ⚔️ Challenge a friend
            </button>
            <button
              onClick={shareResult}
              className="w-full py-4 bg-zinc-800 hover:bg-zinc-700 active:scale-[0.98] text-white font-semibold text-base rounded-xl transition-all"
            >
              Share result
            </button>
            <button
        ...
        {showChallenge && !challengeLink && (
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 text-left">
            ...
            <input
              ...
              className="w-full bg-zinc-800 border border-zinc-700 rounded-xl px-4 py-3 text-white placeholder-zinc-600 focus:outline-none focus:border-yellow-400 transition-colors mb-3"
              autoFocus
            />
            <button
        ...
        {challengeLink && (
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 text-left mb-4">
            ...
            <div className="bg-zinc-800 rounded-xl px-4 py-3 text-zinc-300 text-sm break-all mb-3">
              {challengeLink}
            </div>
            <button
              onClick={() => navigator.clipboard.writeText(challengeLink).then(() => alert('Copied!'))}
              className="w-full py-3 bg-yellow-400 hover:bg-yellow-300 text-black font-bold rounded-xl transition-all"
            >
              Copy link again
            </button>
          </div>
        ...
        {challengeLink && (
          <button onClick={onReplay} className="w-full py-4 bg-zinc-800 hover:bg-zinc-700 text-white font-semibold rounded-xl transition-all mt-3">
            Play again
          </button>
        )}
